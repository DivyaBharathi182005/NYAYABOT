...
function pickVoice(lang) {
  ...
  for (const locale of fallbacks) {
    const femaleExact = voices.find(v => v.lang === locale && femaleVoice(v));
    if (femaleExact) return femaleExact;
    // Exact locale match
    const exact = voices.find(v => v.lang === locale);
    if (exact) return exact;
    // Prefix match (e.g. 'hi' matches 'hi-IN')
    const femalePrefix = voices.find(v => v.lang.startsWith(locale.split('-')[0] + '-') && femaleVoice(v));
    if (femalePrefix) return femalePrefix;
  ...
  return voices.find(v => v.lang.startsWith('en') && femaleVoice(v))
