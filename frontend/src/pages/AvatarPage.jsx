...
export default function AvatarPage() {
  ...
  const speak = useCallback((text) => {
    ...
    const next = () => {
      ...
      const femaleVoice = (voice) => /female|woman|zira|samantha|susan|karen|victoria|heera|kalpana|veena|google uk english female|google हिन्दी/i.test(`${voice.name} ${voice.voiceURI}`);
      const voice = voices.find(v => v.lang === currentLocale && femaleVoice(v))
        || voices.find(v => v.lang === currentLocale)
        || voices.find(v => v.lang.startsWith(language) && femaleVoice(v))
        || voices.find(v => v.lang.startsWith(language))
        || voices.find(v => v.lang.startsWith('en') && femaleVoice(v))
  ...
  const handleSend = useCallback(async (q = query) => {
    ...
    setIsLoading(true);
    try {
      const asksForPolice = /police station|nearest police|nearby police|police near/i.test(text);
      let reply;
      if (asksForPolice) {
        const areaMatch = text.match(/\b(?:in|near|at)\s+(.+)$/i);
        let searchText = areaMatch?.[1]?.replace(/[?.!,]+$/, '').trim();
        let coordinates;
        if (!searchText && navigator.geolocation) {
          const position = await new Promise(resolve => navigator.geolocation.getCurrentPosition(resolve, () => resolve(null)));
          if (position) {
            coordinates = { latitude: position.coords.latitude, longitude: position.coords.longitude };
            searchText = `${position.coords.latitude.toFixed(5)},${position.coords.longitude.toFixed(5)}`;
          }
        }
        if (!searchText) {
          reply = 'Please tell me your area or allow location access so I can find nearby police stations.';
        } else {
          const data = await findPoliceStations(searchText, coordinates);
          const stations = (data.stations || []).slice(0, 3);
          reply = stations.length
            ? `Here are nearby police stations${data.area ? ` near ${data.area.split(',')[0]}` : ''}:\n${stations.map((station, index) => `${index + 1}. ${station.name} - ${station.address || 'Address unavailable'}${station.phone ? ` - ${station.phone}` : ''}`).join('\n')}`
            : 'I could not find a nearby police station. Please try a more specific area or open the Find Police Station module.';
        }
      } else {
        const history = messages.slice(-8).map(message => ({ role: message.role, text: message.text }));
        reply = await chatAssistant(text, language, history);
      }
      ...
      <div style={{ background: '#0d0d1e', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: 20, boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 20, marginBottom: 18 }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={pauseSpeaking} disabled={!isSpeaking} style={{ flex: 1, padding: '7px 0', borderRadius: 9, background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.25)', cursor: isSpeaking ? 'pointer' : 'not-allowed', color: '#c9a84c', fontSize: 12 }}>Pause / Resume</button>
              <button onClick={stopSpeaking} disabled={!isSpeaking} style={{ flex: 1, padding: '7px 0', borderRadius: 9, background: 'rgba(232,92,92,0.08)', border: '1px solid rgba(232,92,92,0.25)', cursor: isSpeaking ? 'pointer' : 'not-allowed', color: '#e85c5c', fontSize: 12 }}>Stop Speaking</button>
            </div>
          </div>
        </div>
