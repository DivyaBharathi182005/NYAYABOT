...
import { matchLawyers, LAWYERS } from './services/lawyerMatcher.js';
dotenv.config();
...
import {
  ...
  retrieveRelevantSectionsSemantic,
  getSectionByNumber,
  getAllSections,
  getStats,
...
const retrievedSections = await retrieveRelevantSectionsSemantic(query, 5);    const analysis = await analyzeLegalQuery({ query, retrievedSections, language });
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const reply = await chatAssistant({ message, language, history });
    res.json({ success: true, reply });
  } catch (err) {
    console.error('General chat error:', err.message);
    res.status(500).json({ error: 'Failed to answer. Check your GROQ_API_KEY.' });
  }
});
...
// ── LAWYER MATCHING (real matched lawyers with fee/phone/email) ──
app.post('/api/lawyer/match', (req, res) => {
  try {
    const { issue } = req.body;
    if (!issue?.trim()) return res.status(400).json({ error: 'Issue description is required' });
    const result = matchLawyers(issue);
    res.json({ success: true, ...result });
  } catch (err) {
    console.error('Lawyer match error:', err.message);
    res.status(500).json({ error: 'Failed to match lawyer' });
  }
});

app.get('/api/lawyer/all', (req, res) => {
