...
let SECTION_EMBEDDINGS = null;
try {
  SECTION_EMBEDDINGS = JSON.parse(
    readFileSync(path.join(__dirname, '../data/ipc_embeddings.json'), 'utf-8')
  );
} catch {
  SECTION_EMBEDDINGS = null; // run buildEmbeddings.js first
}
...
// ── Semantic (Hugging Face) re-ranking on top of TF-IDF ────────────
export async function retrieveRelevantSectionsSemantic(query, topK = 5) {
  const baseResults = retrieveRelevantSections(query, 15); // wider TF-IDF shortlist

  if (!SECTION_EMBEDDINGS || !process.env.HF_API_KEY) {
    return baseResults.slice(0, topK); // graceful fallback, no HF key/cache yet
  }

  try {
    const queryVec = await getEmbedding(query);
    const reranked = baseResults.map(section => {
      const emb = SECTION_EMBEDDINGS.find(e => e.section_number === section.section_number);
      const semScore = emb?.vector ? cosineSimilarity(queryVec, emb.vector) : 0;
      return { ...section, semanticScore: semScore, combinedScore: section.relevanceScore * 0.4 + semScore * 0.6 };
    });
    reranked.sort((a, b) => b.combinedScore - a.combinedScore);
    return reranked.slice(0, topK);
  } catch (err) {
    console.error('HF semantic rerank failed, using TF-IDF only:', err.message);
