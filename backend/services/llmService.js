...
Analyze this legal situation in a compact format:
1. **Primary Applicable Section**: Most relevant IPC/BNS section with number
2. **Legal Summary**: One short sentence describing the offense
3. **Punishment Details**: Exact imprisonment and fine under IPC and BNS, in one or two short sentences
4. **Immediate Steps**: Up to 3 short numbered actions
5. **Important Note**: One short caveat or warning

${language !== 'en' ? `🔴 REMINDER: Respond COMPLETELY in ${LANG_NAMES[language] || language}. No English words except IPC/BNS section numbers.` : ''}

Use clear formatting with bold headers. Keep the complete response under 180 words. Do not include case-strength analysis, landmark cases, or general legal history.`;
...
// General-purpose assistant used by the avatar. Legal questions can still be
// answered, but the response is not forced into an IPC/BNS report format.
export async function chatAssistant({ message, language = 'en', history = [] }) {
  ...
  const conversation = Array.isArray(history)
    ? history.slice(-8).map(item => ({
      ...
      content: String(item.text || '').substring(0, 1200),
    }))
    : [];
  ...
  const response = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are NyayaBot, a friendly general-purpose assistant for people in India. Answer the user's actual question directly and naturally. Do not force every answer into IPC or BNS sections. For legal questions, give concise, plain-language information and mention relevant law only when useful. For practical questions such as finding a nearby police station, explain the quickest useful action and ask for the area or location if needed. Never invent live locations, phone numbers, or current facts. Reply in ${languageName}. Keep responses under 180 words.`,
      },
      ...conversation,
      { role: 'user', content: String(message).substring(0, 4000) },
    ],
  });
...
const LOCAL_TERM_DEFINITIONS = {
  ...
  fir: 'An FIR is the First Information Report recorded by police when they receive information about a cognizable offence. It starts the investigation.',
  chargesheet: 'A chargesheet is the police report filed after investigation, describing the alleged offence, evidence, and accused persons.',
  cognizance: 'Cognizance means a court formally takes notice of an alleged offence and begins considering the case.',
  ...
  acquittal: 'Acquittal is a court finding that the prosecution did not prove the charge beyond the required legal standard.',
};

export async function explainLegalTerm(term, language = 'en') {
  const localDefinition = LOCAL_TERM_DEFINITIONS[term.trim().toLowerCase()];
  if (localDefinition && !process.env.GROQ_API_KEY) return localDefinition;
